"! Lab 06 — RUNTIME-READY VARIANT of the historical zcl_lab_06_condition_fq.
"! Only change: the class-level DATA declarations that used ZEMP_LOGALI as a
"! compile-time DDIC TYPE now use zcl_employee_fixture_fq's own row type, and
"! perform_loop_endloop() populates the table via the fixture instead of
"! SELECT * FROM zemp_logali. Every other method, control-flow demonstration,
"! and the real cx_sy_zerodivide exception handling are unchanged. See
...
CLASS zcl_lab_06_condition_rt_fq DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    DATA: mv_conditional TYPE i,
          ...
          mv_time        TYPE t,
          mv_counter_2   TYPE i,
          mt_employees   TYPE zcl_employee_fixture_fq=>tt_employee_fixture_fq,
          ms_employee    TYPE zcl_employee_fixture_fq=>ty_employee_fixture_fq,
...
CLASS zcl_lab_06_condition_rt_fq IMPLEMENTATION.
  ...
  METHOD perform_loop_endloop.
    me->mt_employees = zcl_employee_fixture_fq=>get_employees( ).
